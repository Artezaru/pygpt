...
latexpdf:
	...
	cd $(BUILDDIR)/latex && pdflatex pygpt.tex && pdflatex pygpt.tex
...
app:
	echo "from pygpt.__main__ import __main_gui__" > run_gui.py
	echo "__main_gui__()" >> run_gui.py
	pyinstaller --name pygpt --onefile --windowed run_gui.py
	rm run_gui.py
	rm -rf build
	rm run_gui.spec
