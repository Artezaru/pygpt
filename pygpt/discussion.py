...
class Discussion:
    ...
    model : str
        The GPT model to use (e.g., "gpt-3.5-turbo" or "gpt-4").

    gpt_api : GPTApi
        An instance of the `GPTApi` class to interact with the GPT model.

    complete_message : Message
        Stores the full conversation.

    resume_message : Message
        Stores the summarized conversation to be sent to GPT.

    Methods
    -------
    add_user_question(question: str) -> str
        Adds a user's question to the discussion and manages the conversation flow.

    load(file_path: str) -> None
        Loads the discussion from a JSON file.

    dump(file_path: str) -> None
        Saves the discussion to a JSON file.

    handle_long_discussion() -> None
